...
class SnapDB {
    ...
    async getCollection(collectionName) {
        return this.logger.wrapWithLogging('getCollection', async () => {
            ...
            let data;
            try {
                data = await csv.parse(filePath, { lineAsArray: false });
            } catch (error) {
        ...
        return this.logger.wrapWithLogging('deleteById', async () => {
            ...
            try {
                if (collectionName) {
                    ...
                    let records;
                    try {
                        records = await csv.parse(collectionPath, PARSE_OPTIONS);
                    } catch (error) {
                        throw new CorruptedDataError(collectionName, error.message);
                    }
                    ...
                    try {
                        await csv.deleteRows(collectionPath, {
                            rowNumber: requestedItemIndex + 1,
                            rowsToDelete: 1,
    ...
    async updateById(id, updates, collectionName = '') {
        ...
        try {
            await this.deleteById(id, targetCollection);
            const updatedData = { ...record, ...updates };
            delete updatedData._collection;
            delete updatedData.id;

            const result = await this.insert(targetCollection, updatedData);
            const finalResult = { ...result, message: 'Record updated successfully' };
